	...
	casper.repeat(/*classes.length*/ 500, function(){
		this.thenOpen(rootUrl + classes[i].href, function(){
			...
			while(this.exists(x('/html/body/div[4]/div[2]//a[@name="constructor.summary"]/../table/tbody/tr[' + constIndex + ']/td/code'))){
				constructor = this.fetchText(x('/html/body/div[4]/div[2]//a[@name="constructor.summary"]/../table/tbody/tr[' + constIndex + ']/td/code')).replace(/\u00a0/g, " ");
				description = this.fetchText(x('/html/body/div[4]/div[2]//a[@name="constructor.summary"]/../table/tbody/tr[' + constIndex + ']/td/div')).replace(/\u00a0/g, " ");
...
casper.then(function(){
	classesString = JSON.stringify(classes, null, 4);
	fs.write("Test.json", classesString, 'w');
});
